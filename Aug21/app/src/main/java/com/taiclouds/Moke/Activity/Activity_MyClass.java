...
import com.taiclouds.Moke.Adapter.Adapter_MyClass;
...
public class Activity_MyClass extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (userName == null | password == null){
            ...
            getData();                      //3.获取数据
            setListViewListener();          //4.设置ListView监听器
        }
    }
    ...
    public void init(){
        //友盟推送的统计
        PushAgent.getInstance(Activity_MyClass.this).onAppStart();
    ...
    public void setListViewListener(){
        myClassListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Intent intent = new Intent();
                intent.setClass(Activity_MyClass.this, Activity_CourseDetail.class);        //跳转到课程详情列表

                Bundle bundle = new Bundle();
                bundle.putString("courseID", courseIDStrings[position]);                           //点击了第几个课程分类
                bundle.putString("cityID", cityNameStrings[position]);
                System.out.println(courseIDStrings[position] + "-->" + cityNameStrings[position]);
