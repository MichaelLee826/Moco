package com.taiclouds.Moke.BusinessLogicLayer.utility;

import android.content.Context;
import android.content.res.AssetManager;
import android.util.Log;

import com.taiclouds.Moke.Activity.Activity_Main;
 ...
 * Modified by Michael on 2016-8-20
 */
public class Utility {
    ...
    public static void readProvinceFromAssets(final Context context, final Activity_Main.MyCallback callback) {
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                AssetManager assetManager = context.getAssets();
                    ...
                    for (City city : cityList) {
                        ...
                        for (Map.Entry<String, String> entry : map.entrySet()) {
                            if (entry.getKey().substring(0, 4).equals(identifier.substring(0, 4))
                                ...
                                //System.out.println("-->" + Long.parseLong(entry.getKey()));
                                List<County> list = countyDao.queryBuilder().where(CountyDao.Properties.Identifier.eq(Long.parseLong(entry.getKey()))).list();
                                for (County c: list) {
                                    //System.out.println("name:"+c.getName()+"cityId:"+c.getCityID()+"identifier:"+c.getIdentifier()+"id:"+c.getId());
                                }
